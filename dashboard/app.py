...
if "mode" not in st.session_state:
    st.session_state["mode"] = "radar"
if "selected_symbol" not in st.session_state:
    st.session_state["selected_symbol"] = ""


# ── Helper: load radar data ───────────────────────────────────────────────────
@st.cache_data(ttl=300)
def load_radar_data():
    """
    Load all stocks from the most recent date in manipulation_scores,
    joined with daily_prices and rolling_stats.
    Returns (df, most_recent_date_str) or (empty_df, None).
    """
    try:
        ...
        date_df = pd.read_sql_query(
            "SELECT MAX(date) AS max_date FROM manipulation_scores", conn
        )
        if date_df.empty or date_df["max_date"].iloc[0] is None:
            conn.close()
            return pd.DataFrame(), None

        most_recent = date_df["max_date"].iloc[0]
        ...
        query = """
            SELECT
                ...
                ms.signal_circuit,
                ms.signal_velocity,
                ms.signal_corp_event,
                ms.signal_pref_allot,
                ms.signal_bulk_deal,
                dp.close,
                dp.pct_change,
                dp.delivery_pct,
            ...
            LEFT JOIN daily_prices dp
                ON ms.symbol = dp.symbol AND ms.date = dp.date
            LEFT JOIN rolling_stats rs
                ON ms.symbol = rs.symbol AND ms.date = rs.date
            WHERE ms.date = ?
            ORDER BY ms.total_score DESC, ms.symbol ASC
        """
        df = pd.read_sql_query(query, conn, params=(most_recent,))
        conn.close()
...
@st.cache_data(ttl=300)
def load_radar_fallback():
    """
    Fallback: load raw daily_prices data when manipulation_scores is empty.
    Returns (df, most_recent_date_str) or (empty_df, None).
    """
    try:
        conn = get_db_connection()
        date_df = pd.read_sql_query(
            "SELECT MAX(date) AS max_date FROM daily_prices", conn
        )
        if date_df.empty or date_df["max_date"].iloc[0] is None:
            conn.close()
            return pd.DataFrame(), None
        ...
        most_recent = date_df["max_date"].iloc[0]
        query = """
            SELECT symbol, close, pct_change, delivery_pct, total_volume
            FROM daily_prices
            WHERE date = ?
            ORDER BY symbol ASC
        """
...
def get_top_signal(row):
    """Return the name of the signal column with the highest value for a row."""
    signal_cols = [
    ...
    if best_col is None or best_val <= 0:
        return "—"
    return best_col.replace("signal_", "").replace("_", " ").title()


def score_color(score):
    """Return a CSS background color string based on score."""
    if score >= 7.0:
        return "background-color: #7b1c1c; color: #ff6b6b;"
    elif score >= 5.0:
        return "background-color: #7b4a00; color: #ffa94d;"
    elif score >= 3.0:
        return "background-color: #5c4a00; color: #ffe066;"
    return ""
...
if st.session_state["mode"] == "radar":
    ...
    with st.sidebar:
        ...
        min_score = st.slider(
            ...
            value=2.0,
            step=0.5,
            help="Filter stocks by minimum manipulation score",
        )

        phase_filter = st.selectbox(
            "Phase Filter",
            options=["All", "Pump", "Distribution", "Extreme"],
            index=0,
        )

        jump_symbol = st.text_input(
            "Jump to Symbol",
            value="",
            placeholder="e.g. RELIANCE",
            help="Type a symbol and press Enter to view its detail page",
        ).strip().upper()

        if jump_symbol:
            st.session_state["selected_symbol"] = jump_symbol
            st.session_state["mode"] = "detail"
            st.rerun()

        if st.button("🔍 View All Stocks", use_container_width=True):
            min_score = 0.0
            st.session_state["_radar_min_score_override"] = 0.0
    ...
    if "_radar_min_score_override" in st.session_state:
        min_score = st.session_state.pop("_radar_min_score_override")

    # ── Load data ─────────────────────────────────────────────────────────────
    radar_df, most_recent_date = load_radar_data()

    # ── Page header ───────────────────────────────────────────────────────────
    st.title("🚨 Manipulation Radar")
    if most_recent_date:
        st.markdown(f"**NSE Stocks Ranked by Manipulation Risk — {most_recent_date}**")
    else:
        st.markdown("**NSE Stocks Ranked by Manipulation Risk**")

    # ── Handle empty scores table ─────────────────────────────────────────────
    if radar_df.empty:
        st.warning(
            "No scores available yet. Run `python3 src/scoring_engine.py` to generate today's scores."
        )

        # Fallback: show raw daily_prices data
        fallback_df, fallback_date = load_radar_fallback()
        if not fallback_df.empty:
            st.info(
                f"📊 Showing raw price data from `daily_prices` for {fallback_date} "
                "(scores pending — run scoring engine to populate manipulation scores)."
            )
            st.dataframe(fallback_df, use_container_width=True, hide_index=True)
        st.stop()

    # ── Metric boxes ──────────────────────────────────────────────────────────
    today_df = radar_df  # already filtered to most recent date

    extreme_count = int((today_df["total_score"] >= 7.0).sum())
    distrib_count = int(((today_df["total_score"] >= 5.0) & (today_df["total_score"] < 7.0)).sum())
    pump_count    = int(((today_df["total_score"] >= 3.0) & (today_df["total_score"] < 5.0)).sum())
    clean_count   = int((today_df["total_score"] < 3.0).sum())

    m1, m2, m3, m4 = st.columns(4)
    with m1:
        st.metric("🚨 Extreme (7+)", extreme_count)
    with m2:
        st.metric("🔴 Distribution (5–7)", distrib_count)
    with m3:
        st.metric("🟡 Pump Phase (3–5)", pump_count)
    with m4:
        st.metric("✅ Clean (<3)", clean_count)

    st.markdown("---")

    # ── Apply filters ─────────────────────────────────────────────────────────
    filtered_df = today_df[today_df["total_score"] >= min_score].copy()
    ...
    if phase_filter != "All":
        ...
        target_phase = phase_map.get(phase_filter, phase_filter.upper())
        filtered_df = filtered_df[
            filtered_df["phase"].str.upper() == target_phase
        ]

    if filtered_df.empty:
        st.info(
            f"No stocks found with score ≥ {min_score}"
            + (f" and phase = {phase_filter}" if phase_filter != "All" else "")
            + f". The highest score today is **{today_df['total_score'].max():.1f}**. "
            "Try lowering the minimum score slider or click **🔍 View All Stocks**."
        )
        st.stop()
    ...
    filtered_df["Top Signal"] = filtered_df.apply(get_top_signal, axis=1)
    ...
    display_cols = {
        ...
        "close": "Close Price",
        "pct_change": "% Change",
        "vol_ratio": "Vol/Avg Ratio",
        "delivery_pct": "Delivery %",
        "Top Signal": "Top Signal Fired",
    }

    table_df = filtered_df[list(display_cols.keys())].rename(columns=display_cols)

    # Format numeric columns
    if "Close Price" in table_df.columns:
        table_df["Close Price"] = table_df["Close Price"].apply(
    ...
    if "Vol/Avg Ratio" in table_df.columns:
        table_df["Vol/Avg Ratio"] = table_df["Vol/Avg Ratio"].apply(
            lambda x: f"{x:.2f}x" if pd.notna(x) else "N/A"
        )
    if "Delivery %" in table_df.columns:
        table_df["Delivery %"] = table_df["Delivery %"].apply(
            lambda x: f"{x:.1f}%" if pd.notna(x) else "N/A"
        )

    st.subheader(f"📋 Ranked Stocks — {len(table_df)} shown (min score: {min_score})")

    # ── Render table with per-row detail buttons ──────────────────────────────
    # Header row
    hcols = st.columns([1, 2, 1.2, 1.5, 1.5, 1.2, 1.5, 1.3, 2, 1.5])
    headers = ["Rank", "Symbol", "Score", "Phase", "Close Price",
               "% Change", "Vol/Avg Ratio", "Delivery %", "Top Signal Fired", "Action"]
    ...
    for _, row in table_df.iterrows():
        score_val = filtered_df.loc[
            filtered_df["Rank"] == row["Rank"], "total_score"
        ].values[0]

        rcols = st.columns([1, 2, 1.2, 1.5, 1.5, 1.2, 1.5, 1.3, 2, 1.5])
        rcols[0].write(str(row["Rank"]))
        rcols[1].write(f"**{row['Symbol']}**")

        # Score with color badge
        if score_val >= 7.0:
            rcols[2].markdown(
                f"<span style='background:#7b1c1c;color:#ff6b6b;padding:2px 6px;border-radius:4px;font-weight:bold'>{score_val:.2f}</span>",
                unsafe_allow_html=True,
            )
        elif score_val >= 5.0:
            rcols[2].markdown(
                f"<span style='background:#7b4a00;color:#ffa94d;padding:2px 6px;border-radius:4px;font-weight:bold'>{score_val:.2f}</span>",
                unsafe_allow_html=True,
            )
        elif score_val >= 3.0:
            rcols[2].markdown(
                f"<span style='background:#5c4a00;color:#ffe066;padding:2px 6px;border-radius:4px;font-weight:bold'>{score_val:.2f}</span>",
                unsafe_allow_html=True,
            )
        else:
            rcols[2].write(f"{score_val:.2f}")

        rcols[3].write(str(row["Phase"]))
        rcols[4].write(str(row["Close Price"]))
        rcols[5].write(str(row["% Change"]))
        rcols[6].write(str(row["Vol/Avg Ratio"]))
        rcols[7].write(str(row["Delivery %"]))
        rcols[8].write(str(row["Top Signal Fired"]))

        if rcols[9].button("🔍 View", key=f"btn_{row['Symbol']}"):
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODE 2 — STOCK DETAIL PAGE (existing dashboard, untouched logic)
# ═══════════════════════════════════════════════════════════════════════════════
else:  # mode == "detail"
    ...
    with st.sidebar:
        ...
        symbols = get_symbols()
        if symbols:
            # Pre-select the symbol from session state if available
            preselected = st.session_state.get("selected_symbol", "")
            if preselected and preselected in symbols:
                default_idx = symbols.index(preselected)
            else:
                default_idx = 0
